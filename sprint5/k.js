...
function printRange(root, left, right) {
  if (root.left && root.value >= left) printRange(root.left, left, right);
  if (root.value >= left && root.value <= right) console.log(root.value);
  if (root.right && root.value <= right) printRange(root.right, left, right);
}
